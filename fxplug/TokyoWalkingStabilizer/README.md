...
- Version `1.1.1` keeps the adopted baseline for far-field micro-shake correction.
  Keep schema 45's fps-derived dominant `5x9` mesh windows as the primary evidence
  for short source-space lens/camera shake up to one second, with `3x5`
  source-lens local evidence feeding the coherent low-order correction. Do not
  return to a fixed 10-frame detector or a visible local mesh warp that makes
  mountains, clouds, or horizons pulse.
- Requires schema 45 far-field `5x9` mesh paths, fps-derived dominant mesh shake
  windows, `3x5` source-lens local paths, two-way far-field rigid shake paths,
  lens-band evidence, search-radius edge-hit counts, and chunked fingerprints.
  Schema 45 is the only current write/read format. Earlier caches are intentionally
...
- Schema 45 analysis uses a `5x9` far-field mesh over the upper-frame cloud,
  ...
  making the mountain shape locally swim. Earlier schemas are unsupported and
  require a fresh Event Analyzer run.
- Earlier schema history is retained only as implementation context. Current
  runtime does not synthesize missing rigid, mesh, or dominant-window evidence
  from older caches; rerun the Event Analyzer to produce schema 45.

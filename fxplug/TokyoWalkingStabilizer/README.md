...
- Version `1.1.19` keeps the all-axis Camera Jitter baseline on schema 49.
  Keep schema 49's fps-derived dominant `5x9` mesh windows as supporting evidence
...
- Requires schema 49 far-field `5x9` mesh paths, fps-derived dominant mesh shake
  ...
  Schema 49 is the only current write/read format. Earlier caches are intentionally
...
- Schema 49 analysis uses a `5x9` far-field mesh over the upper-frame cloud,
...
- Earlier schema history is retained only as implementation context. Current
  ...
  evidence from older caches; rerun the Event Analyzer to produce schema 49.

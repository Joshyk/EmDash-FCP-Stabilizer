...
- Version `1.1.20` keeps the all-axis Camera Jitter baseline on schema 50.
  Schema 50 stores direct frame-local X/Y/roll targets and replaces temporal
  sign matching with independent top/ridge regional agreement. The playback
  trajectory applies a symmetric alternating-Y filter only to non-rigid Camera
  Jitter components.
  Keep schema 50's fps-derived dominant `5x9` mesh windows as supporting evidence
...
- Requires schema 50 far-field `5x9` mesh paths, fps-derived dominant mesh shake
  ...
  Schema 50 is the only current write/read format. Earlier caches are intentionally
...
- Schema 50 analysis uses a `5x9` far-field mesh over the upper-frame cloud,
...
- Earlier schema history is retained only as implementation context. Current
  ...
  evidence from older caches; rerun the Event Analyzer to produce schema 50.

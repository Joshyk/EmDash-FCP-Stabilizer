...
- Schema 43 analysis adds a 5x5 far-field mesh over the upper-frame cloud,
  ridge/horizon, and distant-mountain area. Render aggregates that denser
  evidence into the existing rigid source-space correction instead of applying a
  local mesh warp, so it can see smaller 10-frame camera/lens shakes without
  making the mountain shape locally swim. Schema 42 caches remain valid but do
  not provide this mesh evidence.
- Schema 42 analysis adds two-way far-field rigid shake paths for the cloud/top,
  ridge/horizon, and distant-mountain bands. Render uses that evidence first so
  the far field moves as one plane. Schema 41 caches synthesize the same rigid
  paths in memory at load time; in both cases, the older local differential band
  warp is suppressed instead of silently reappearing and pulsing mountains or clouds.

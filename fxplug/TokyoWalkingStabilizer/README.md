...
- `Debug Overlay`: normally off. When enabled, the labeled top-left bars show `X`, `Y`,
  ...
  runtime/source diagnostics so Final Cut Pro runtime analysis can be checked. `R357` means
  FxPlug `0.3.57` is rendering original/optimized frames, and `P357` means proxy playback is

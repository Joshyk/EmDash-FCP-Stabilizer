...
- Version `1.2.9` maps Turn Smoothing Strength linearly in viewport space: 12 equals the former 36 result and 36 applies three times that zoom and X movement. The Turn Transition Window is a fixed maximum accumulation span measured from the first active same-direction sample; later samples do not roll it forward. Final composed Viewer X uses one constant cruising velocity, with quintic easing limited to up to 0.30 seconds at each endpoint. If the Window cap splits uninterrupted same-direction activity, the bounded events share one render chain and do not stop at the boundary. TURN macro X remains its travel authority. Endpoint adjustment carries through a continuous handoff, but during a true idle span it is released using the same quintic duration as `Auto Crop Zoom-Out Time` (clamped by the available idle span) back to the underlying X path, so Crop Off does not retain a stationary black edge. Brief opposite-sign activity is absorbed by travel-based reversal hysteresis that rises with Strength; substantial physical reversals still start a new event. Diagnostic X components remain visible while continuity correction owns the final sum. Overlapping Auto Crop intervals now hand a higher peak's zoom and crop X down together to the lower next protected scale and X reservation over `min(Zoom-Out Time, time to next peak)` without returning to `1.0x` and center. Both use one constant cruising progress with quintic easing limited to 0.30 seconds at the endpoints; coverage and final framing floors remain absolute. Auto Crop Zoom-In and Zoom-Out default to 6 seconds and Hold remains between them.
  ...
  Keep schema 52's fps-derived dominant `5x9` mesh windows as supporting evidence
  ...
  translation shape. Coherent X/Y/roll bypasses the broad Turn/Macro Jitter smoother. Do not
  return to a fixed 10-frame detector or a visible local mesh warp that makes
  mountains, clouds, or horizons pulse.
- Requires schema 52 far-field `5x9` mesh paths, fps-derived dominant mesh shake
  ...
  Schema 52 is the only current write/read format. Earlier caches are intentionally
...
- Schema 52 analysis uses a `5x9` far-field mesh over the upper-frame cloud,
...
- Earlier schema history is retained only as implementation context. Current
  ...
  evidence from older caches; rerun the Event Analyzer to produce schema 52.
...
- `Camera Jitter X/Y Max Correction (%)` and `Camera Jitter ROLL Max Correction (°)`: Camera
  ...
  Micro and Macro retain their prepared short/medium detection and share these inputs without
...
- `Turn Transition Window (s)`: sets the maximum accumulation span from the first active
  ...
  event. Directly adjacent same-direction events share one render chain. During a true idle span,
  endpoint X correction releases with the same quintic duration as `Auto Crop Zoom-Out Time` (clamped by the available idle span) back to the underlying path;
  immediate handoffs and reversals retain it. It is independent from correction amplitude. `Turn Smoothing Strength` defaults

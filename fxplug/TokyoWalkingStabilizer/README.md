...
- Requires schema 48 far-field `5x9` mesh paths, fps-derived dominant mesh shake
  ...
  Schema 48 is the only current write/read format. Earlier caches are intentionally
...
- Schema 48 analysis uses a `5x9` far-field mesh over the upper-frame cloud,
  ...
  rigid X/Y/roll path uses top/ridge evidence instead of lower mountain/mid-band
  parallax, while render applies Y and roll as global source-space transform and
...
- Earlier schema history is retained only as implementation context. Current
  ...
  evidence from older caches; rerun the Event Analyzer to produce schema 48.
...
- `Turn Smoothing Strength`: controls large segmented walking turns in X translation only.
  ...
  scale. The reservation grows with Turn Smoothing Strength and reaches half the output
  width at `36`, exposing the needed black edge before and through the turn.

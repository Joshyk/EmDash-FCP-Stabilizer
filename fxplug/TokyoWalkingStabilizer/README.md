...
- `Debug Overlay`: normally off. When enabled, the labeled top-left bars show `X`, `Y`,
  ...
  runtime/source diagnostics so Final Cut Pro runtime analysis can be checked. `R360` means
  FxPlug `0.3.60` is rendering original/optimized frames, and `P360` means proxy playback is

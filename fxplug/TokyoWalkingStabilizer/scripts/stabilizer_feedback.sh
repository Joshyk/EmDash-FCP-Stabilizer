...
SOURCE_HASH=$(shasum -a 256 "$SOURCE" "$TURN_TRANSITION_SOURCE" | shasum -a 256 | awk '{print $1}')
...
if [ ! -x "$BINARY" ]; then
	...
	xcrun swiftc "$TURN_TRANSITION_SOURCE" "$TMP_BUILD_DIR/main.swift" -o "$TMP_BINARY"
	chmod +x "$TMP_BINARY"

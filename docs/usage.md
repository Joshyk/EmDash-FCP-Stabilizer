...
Use schema 45 analysis for review builds so the fps-derived dominant `5x9`
mesh windows and `3x5` source-lens local evidence are present. Earlier schemas
...
- Host Analysis cache schema `45` stores the original-size-percentage sample path with the
  ...
  edge-hit counts. Schema 45 is the only current write/read format so playback can use
  denser far-field evidence without returning to visible local mountain/cloud warp.

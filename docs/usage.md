...
Version `1.2.12` is the current review baseline for unrestricted tracked render-space X correction and stable Auto Crop X reservations. Far-field Pan Band X has no 5px limiter, Macro X uses only the explicit tracking-outlier rejection, and Auto Crop required scale has no fixed 128x ceiling.
...
- `Debug Overlay`: a fixed 19-row top-left diagnostic contract, ordered as
  ...
  then the runtime/source row. `ORIGINAL <version>` means the current FxPlug runtime is rendering
  ...
  Remove Black Edges is off. `TRACKING`, `WALKING`, `SHARPNESS`, `RESIDUAL`, and `SEARCH HEADROOM` are quality bars:
...
- Playback uses prepared motion paths from completed analysis. It must not run full
  ...
  jerk limiter used by broader pan and turn stages. Those raw micro paths and their
...
- Host Analysis cache schema `49` stores the original-size-percentage sample path with the
  ...
  edge-hit counts, and validated original-media provenance. Schema 52 is the only current

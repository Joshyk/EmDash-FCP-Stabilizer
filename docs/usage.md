...
Version `1.1.23` is the current review baseline for all-axis Camera Jitter.
...
Camera Rigid Y is bounded relative to render height rather than by a fixed
source-pixel cap, then scaled consistently for proxy diagnostics and rendering.

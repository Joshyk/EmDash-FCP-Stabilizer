...
Version `1.1.19` is the current review baseline for all-axis Camera Jitter.
Use schema 49 analysis so axis-specific top/ridge rigid X/Y/roll support,
...
- Host Analysis cache schema `49` stores the original-size-percentage sample path with the
  ...
  edge-hit counts, and validated original-media provenance. Schema 49 is the only current

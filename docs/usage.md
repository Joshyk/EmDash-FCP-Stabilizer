...
- `Debug Overlay`: labeled top-left diagnostics for final `X`/`Y`/`ROLL`, `FJIT`, `SWOB`,
  ...
  checking runtime behavior. `R359` means FxPlug `0.3.59` is rendering original/optimized
  frames, while `P359` means proxy playback is using the saved Host Analysis path.

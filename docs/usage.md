...
- `Auto Crop Transition Duration`: uses the selected seconds as the Auto Crop
  ...
  when `Remove Black Edges` is on. Original/optimized media keeps the full sampling
  profile; proxy or scaled preview uses fewer quantized samples for smoother realtime
  playback at high duration values.

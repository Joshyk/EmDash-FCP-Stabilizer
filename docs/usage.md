...
Version `1.1.18` is the current review baseline for far-field micro-shake.
...
Analyzer.
During playback, high-confidence rigid-Y evidence follows the prepared coherent
Y path instead of being reduced by the ordinary local-lens step limiter.
...
- Host Analysis cache schema `48` stores the original-size-percentage sample path with the
  ...
  edge-hit counts. Schema 48 is the only current write/read format so playback can use
  denser far-field evidence and coherent global Y/roll correction without returning to

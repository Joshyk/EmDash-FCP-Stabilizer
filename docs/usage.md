...
- `Auto Crop Hold Time`: retained for parameter compatibility and now defines the
  ...
  sit near identity trim their residual zoom delta to about one quarter, so subtle
  or nearly idle sections do not remain as visibly cropped while strong turn
  peaks keep their full planned zoom. When no keypoint is active and the

...
Version `1.1.20` is the current review baseline for all-axis Camera Jitter.
Use schema 50 analysis so frame-local X/Y/roll targets, axis-specific top/ridge support,
and sign-reversing short-period motion are authoritative. A final symmetric
...
- Host Analysis cache schema `49` stores the original-size-percentage sample path with the
  ...
  edge-hit counts, and validated original-media provenance. Schema 50 is the only current

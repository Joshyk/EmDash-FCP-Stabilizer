...
kernels. Schema 48 analysis persists a `5x9` far-field mesh plus fps-derived
...
evidence. The rigid far-field path is now built from the sky/top and
ridge/horizon bands and carries global roll in degrees so lower
mountain/mid-band parallax does not make the mountain shape split or bulge. The
...
now accepts schema 48 only; earlier caches must be regenerated by the Event
Analyzer. Version `1.1.4` makes this schema 48 fps-derived multi-window detector
and rigid Y/roll path the baseline for future far-field shake work; new changes

...
kernels. Schema 52 analysis persists a `5x9` far-field mesh plus fps-derived
dominant mesh shake windows from 1/20s through 1.0s, in addition to the two-way
far-field rigid shake X/Y/roll paths, `3x5` source-lens local paths, and lens-band
evidence. The rigid far-field path uses frame-local, scale-aware top/ridge
agreement, independent forward/backward neighbor checks, and carries global roll in degrees; accumulated mid-band parallax does
not suppress coherent distant motion. The
FxPlug render path can inspect denser upper-frame motion while still applying a
coherent, low-order correction that avoids local mountain/cloud pulsing. Runtime
now accepts schema 52 only; earlier caches must be regenerated by the Event
Analyzer. Version `1.1.26` reuses schema 52 and applies Inspector Camera Rigid
limits as output-relative X/Y percentages and a roll-degree limit at render time.
It stores frame-local X/Y/roll targets and uses each
axis support once instead of repeatedly multiplying the same regional evidence. It
removes a centered one-second quadratic baseline from X before writing the Camera
Rigid target, so broad pan remains Turn-owned while short X reversals remain local.
It also keeps original-media
provenance the baseline for future far-field shake work; new changes

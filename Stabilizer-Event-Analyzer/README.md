...
kernels. Schema 50 analysis persists a `5x9` far-field mesh plus fps-derived
...
evidence. The rigid far-field path uses frame-local, axis-specific top/ridge
agreement and carries global roll in degrees; accumulated mid-band parallax does
...
now accepts schema 50 only; earlier caches must be regenerated by the Event
Analyzer. Version `1.1.20` stores frame-local X/Y/roll targets and makes regional
top/ridge agreement, rather than temporal sign agreement, the axis authority. It
also keeps original-media

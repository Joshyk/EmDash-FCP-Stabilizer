...
paths, two-way far-field rigid shake paths, and fingerprints instead of every
frame's full luma sample. Schema 42 is the current write format so render can
suppress older local band-warp production paths when rigid far-field evidence is
available. Schema 41 caches are read with an explicit in-memory rigid-path
compatibility upgrade so existing timelines can keep playing after a runtime
update; older schemas must be regenerated by the Event Analyzer.

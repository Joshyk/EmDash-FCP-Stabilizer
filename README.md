...
paths, two-way far-field rigid shake paths, far-field 5x5 mesh paths, fps-derived
dominant far-field shake windows up to one second, and fingerprints instead of
every frame's full luma sample. Schema 44 is the current write format so render
can use denser far-field evidence while keeping the visible correction low-order
enough to avoid local mountain/cloud pulsing. Schema 43 caches remain readable
with mesh-only evidence but do not carry the fps-derived dominant window fields.
Schema 41 and 42 caches are read with explicit compatibility behavior so
existing timelines can keep playing after a runtime update; older schemas must be
regenerated by the Event Analyzer.

...
paths, two-way far-field rigid shake paths, far-field `5x9` mesh paths,
fps-derived dominant far-field shake windows up to one second, and fingerprints
instead of every frame's full luma sample. Schema 45 is the current write and
read format so render can use denser far-field evidence while keeping the visible
correction low-order enough to avoid local mountain/cloud pulsing. Earlier

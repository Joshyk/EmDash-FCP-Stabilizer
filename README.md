...
paths, two-way far-field rigid shake paths, far-field 5x5 mesh paths, and
fingerprints instead of every frame's full luma sample. Schema 43 is the current
write format so render can use denser far-field evidence while keeping the
visible correction low-order enough to avoid local mountain/cloud pulsing. Schema
41 and 42 caches are read with explicit compatibility behavior so existing
timelines can keep playing after a runtime update; older schemas must be
